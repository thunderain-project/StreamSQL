...
private[streaming]
class KafkaInputDStreamStub[K: ClassTag, V: ClassTag](
    @transient ssc_ : StreamingContext,
    kafkaParams: Map[String, String],
    topics: Map[String, Int],
  ...
  def getReceiver(): NetworkReceiver[(K, V)] = {
    new KafkaReceiverStub[K, V](kafkaParams, topics, keyDecoder, valueDecoder, storageLevel)
        .asInstanceOf[NetworkReceiver[(K, V)]]
  }
}

private[streaming]
class KafkaReceiverStub[K: ClassTag, V: ClassTag](
    kafkaParams: Map[String, String],
    topics: Map[String, Int],
    keyDecoder: Decoder[K],
    valueDecoder: Decoder[V],
